...
SCENARIOS = {
    "sequential_write": {"name": "sequential_write", "rwkind": "write", "blocksize": "512k"},
    "sequential_read": {"name": "sequential_read", "rwkind": "read", "blocksize": "512k"},
    "random_write": {"name": "random_write", "rwkind": "randwrite", "blocksize": "4k"},
    "random_read": {"name": "random_read", "rwkind": "randread", "blocksize": "4k"},
    "random_read_write": {"name": "random_read_write", "rwkind": "randrw", "blocksize": "4k"},
    "sequential_trim": {"name": "sequential_trim", "rwkind": "trim", "blocksize": "512k"},
...
AGAINST_DEVICE_MODES = {AGAINST_DEVICE_WITH_FILL_MODE, AGAINST_DEVICE_WITHOUT_FILL_MODE}
FILL_TARGET_MODES = {AGAINST_DEVICE_WITH_FILL_MODE, AGAINST_FILE_WITH_FILL_MODE}
...
flags.DEFINE_integer(
    "fio_runtime", 600, "The number of seconds to run each fio job for.", lower_bound=1
)
flags.DEFINE_integer(
    "fio_ramptime",
    10,
...
flags.DEFINE_boolean("fio_lat_log", False, "Whether to collect a latency log of the fio jobs.")
flags.DEFINE_boolean("fio_bw_log", False, "Whether to collect a bandwidth log of the fio jobs.")
flags.DEFINE_boolean("fio_iops_log", False, "Whether to collect an IOPS log of the fio jobs.")
flags.DEFINE_integer(
    "fio_log_avg_msec",
    1000,
...
flags.DEFINE_integer("fio_command_timeout_sec", None, "Timeout for fio commands in seconds.")
flags.DEFINE_enum(
    "fio_rng",
    "tausworthe64",
...
_FIO_RATE_BANDWIDTH_LIMIT = flags.DEFINE_string(
    ...
    None,
    "The bandwidth cap in bytes/sec. For example, using " "rate=1m caps bandwidth to 1MiB/sec.",
)
...
ALL_OPERATIONS = frozenset([OPERATION_READ, OPERATION_WRITE, OPERATION_TRIM, OPERATION_READWRITE])
...
MAP_ACCESS_OP_TO_RWKIND = {
    ...
    (ACCESS_PATTERN_SEQUENTIAL, OPERATION_READWRITE): RWKIND_SEQUENTIAL_READ_WRITE,
...
def GetScenarioFromScenarioString(scenario_string):
    ...
    result = SCENARIOS.get(scenario_string, None) if FLAGS.fio_use_default_scenarios else None
    if result:
    ...
    if access_pattern not in ALL_ACCESS_PATTERNS:
        raise errors.Setup.InvalidFlagConfigurationError(
            f"Unexpected access pattern {access_pattern} " f"in scenario {scenario_string}"
        )

    if operation not in ALL_OPERATIONS:
        raise errors.Setup.InvalidFlagConfigurationError(
            f"Unexpected operation {operation}" f"in scenario {scenario_string}"
        )
    ...
    result = {"name": scenario_string.replace(",", "__"), "rwkind": rwkind, "size": workingset_str}
    ...
    for extra_fields in fields[4:]:
        ...
        if key not in FIO_KNOWN_FIELDS_IN_JINJA:
            raise errors.Setup.InvalidFlagConfigurationError(
                "Unregonzied FIO parameter {0} out of scenario {1}".format(key, scenario_string)
            )
        result[key] = value
...
def GenerateJobFileString(
    ...
    blocksize_override = str(int(block_size.m_as(units.byte))) + "B" if block_size else None
    ...
    if should_use_scenario_iodepth_numjobs:
        ...
        for scenario in scenarios:
            scenario["name"] = scenario["name"].replace(f'_iodepth-{scenario["iodepth"]}', "")
            scenario["name"] = scenario["name"].replace(f'_numjobs-{scenario["numjobs"]}', "")
    ...
    job_file_template = jinja2.Template(JOB_FILE_TEMPLATE, undefined=jinja2.StrictUndefined)
...
def GetOrGenerateJobFileString(
    ...
    if use_user_jobfile:
        ...
        return ProcessedJobFileString(user_job_file_string or job_file_contents, remove_filename)
    else:
        if against_device:
...
NEED_SIZE_MESSAGE = (
    "You must specify the working set size when using " "generated scenarios with a filesystem."
)
...
def WarnOnBadFlags():
    ...
    if FLAGS.fio_jobfile:
        ...
        if ignored_flags:
            logging.warning(
                'Fio job file specified. Ignoring options "%s"', ", ".join(ignored_flags)
            )
...
def GetLogFlags(log_file_base):
    ...
    if FLAGS.fio_hist_log:
        fio_command_flags = " ".join([fio_command_flags, "--log_hist_msec=%(hist_interval)d"])
...
def RunWithExec(vm, exec_path, remote_job_file_path, job_file_contents):
    ...
    collect_logs = any(
        [FLAGS.fio_lat_log, FLAGS.fio_bw_log, FLAGS.fio_iops_log, FLAGS.fio_hist_log]
    )
    ...
    stdout, _ = vm.RobustRemoteCommand(fio_command, timeout=FLAGS.fio_command_timeout_sec)
    end_time = time.time()
    bin_vals = []
    if collect_logs:
        vm.PullFile(vm_util.GetTempDir(), "%s*.log" % log_file_base)
        if FLAGS.fio_hist_log:
            num_logs = int(vm.RemoteCommand("ls %s_clat_hist.*.log | wc -l" % log_file_base)[0])
            bin_vals += [
                fio.ComputeHistogramBinVals(vm, "%s_clat_hist.%s.log" % (log_file_base, idx + 1))
